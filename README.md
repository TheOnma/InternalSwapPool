...
1. **Checks swap direction**: Determines if trader is selling token0 or token1
2. **Calculates fillable amount**: Uses `SwapMath.computeSwapStep()` to determine how much can be filled from fees
3. **Fills order internally**: Updates `BeforeSwapDelta` to reduce the swap amount
4. **Syncs balances**: Transfers tokens between hook and PoolManager
...
1. **Calculates fee**: Takes 1% of the swap output amount
2. **Deposits fee**: Stores fee in `_poolFees` mapping
3. **Distributes fees**: Calls `donate()` if threshold reached
4. **Updates delta**: Reduces user's received amount by fee
...
```solidity
// Minimum amount required before fees are donated to LPs
...
|------|---------|---------|
| `beforeSwap` | ✅ | Fill orders from collected fees |
| `afterSwap` | ✅ | Collect swap fees and distribute |
| `beforeSwapReturnDelta` | ✅ | Modify swap amounts |
| `afterSwapReturnDelta` | ✅ | Deduct fees from output |
| Others | ❌ | Not required |
...
**Example:**
```solidity
InternalSwapPool hook = new InternalSwapPool(
...
```solidity
ClaimableFees memory fees = hook.poolFees(poolKey);
...
```solidity
// 1. Deploy the hook (must match CREATE2 address requirements)
...
```solidity
// The hook automatically intercepts swaps
...
```solidity
// User specifies output token amount
...
```solidity
// User specifies input ETH amount
...
```solidity
// Calculate 1% fee on received amount
...
1. **Hook Address Requirements**: Must deploy to an address matching the hook permissions bitmap (Uniswap V4 requirement)
2. **Minimum Donation**: Small pools may take time to accumulate sufficient fees
3. **Single-Sided Donations**: Currently only donates token0 (native token)
4. **No Fee Withdrawal**: Accumulated fees can only be donated to LPs, not withdrawn
...
1. **Batch swaps** when possible to benefit from accumulated fees
2. **Monitor donation threshold** - higher thresholds = fewer donation transactions
3. **Use exact input swaps** for better fee utilization prediction
...
- [Uniswap V4 Hooks Guide](https://github.com/Uniswap/v4-core)
- [BaseHook Implementation](https://github.com/Uniswap/v4-periphery)
